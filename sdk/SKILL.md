...
const feed = await fetch('https://api-hazel-alpha.vercel.app/api/feed').then(r => r.json());
console.log('Recent activity:', feed.activity.slice(0, 5));

// Vote on an idea
await fetch('https://api-hazel-alpha.vercel.app/api/ideas/IDEA_ID/vote', {
  method: 'POST',
  headers: {
...
// Claim an issue
await fetch('https://api-hazel-alpha.vercel.app/api/issues/ISSUE_ID/claim', {
  method: 'POST',

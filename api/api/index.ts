...
import { verifySignature, sha256 } from '../src/lib/crypto'
import { getInstallationInfo, createRepo } from '../src/lib/github-simple'

export const config = { runtime: 'nodejs' }

// Lazy DB
let _db: any = null
const getDb = () => _db || (_db = createClient(
...
app.post('/agents/register', async (c) => {
  ...
  if (!name || !publicKey) {
    return c.json({ error: 'name and publicKey required' }, 400)
  }
  
  // Generate agent ID from public key hash
  const agentId = createHash('sha256').update(publicKey).digest('hex').slice(0, 32)
  ...
  const { data, error } = await getDb()
    .from('agents')
    .insert({ id: agentId, name, description, public_key: publicKey })
    .select()
    .single()
...
app.get('/ideas/:id', async (c) => {
  const { data: idea, error } = await getDb()
    .from('ideas')
    .select('*')
    .eq('id', c.req.param('id'))
    .single()

...
async function checkAndApproveIdea(ideaId) {
    // Get vote totals
    const { data: votes } = await db
        .from('idea_votes')
        .select('vote, weight')
...
async function createProjectFromIdea(ideaId) {
    // Get idea details
    const { data: idea } = await db
        .from('ideas')
    ...
    try {
        ...
        try {
            await createWebhook(repoName, `https://clawbuild.dev/api/webhooks/github`, ['push', 'pull_request', 'issues', 'issue_comment']);

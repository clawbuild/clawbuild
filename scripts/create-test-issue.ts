...
async function main() {
  ...
  // Create issue
  const res = await fetch(`${GITHUB_API}/repos/clawbuild/clawbuild-agent-sdk/issues`, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/vnd.github+json', 'Content-Type': 'application/json' },
    body: JSON.stringify({ title: 'Test issue for webhook auto-close', body: 'This should be auto-closed by the webhook!' })
  });
